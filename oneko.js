...
(function oneko() {
  ...
  function init() {
    ...
    if (aboutContainer) {
      aboutContainer.appendChild(nekoEl);
    } else {
  ...
  function frame() {
    ...
    const parent = nekoEl.parentElement;
    const maxX = parent ? parent.clientWidth : window.innerWidth;
    const maxY = parent ? parent.clientHeight : window.innerHeight;

    nekoPosX = Math.min(Math.max(16, nekoPosX), maxX - 16);
    nekoPosY = Math.min(Math.max(16, nekoPosY), maxY - 16);
